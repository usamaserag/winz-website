    ...
    <div className="flex gap-4">
      ...
      <div className="flex flex-col items-center">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 shadow
          ${isCompleted ? 'bg-brand-500 text-white' : isActive ? 'bg-brand-100 border-2 border-brand-500 text-brand-600' : 'bg-gray-100 text-gray-400'}`}>
          {icon}
        </div>
        {!isLast && (
          <div className={`w-0.5 flex-1 my-1 min-h-8 ${isCompleted ? 'bg-brand-400' : 'bg-gray-200'}`} />
        )}
      </div>

      {/* content */}
      <div className={`pb-8 ${isLast ? '' : ''}`}>
        <p className={`font-bold text-sm mb-0.5 ${isCompleted ? 'text-brand-600' : isActive ? 'text-gray-900' : 'text-gray-400'}`}>
          {title}
        </p>
...
const Tracking = () => {
  ...
  const features = [
    { key: 'realtime',      icon: <Zap     className="w-7 h-7 text-brand-500" /> },
    { key: 'notifications', icon: <Bell    className="w-7 h-7 text-brand-500" /> },
    { key: 'history',       icon: <History className="w-7 h-7 text-brand-500" /> },
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {/* ─────────────── HERO + SEARCH ─────────────── */}
      <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-gray-900 via-brand-900 to-gray-900 pt-28 pb-20">
        {/* decorative rings */}
        ...
        <div className="relative z-10 w-full max-w-3xl mx-auto px-4 text-center">
          ...
          >
              ...
              <div className="flex flex-col sm:flex-row gap-3 bg-white/10 backdrop-blur-md p-3 rounded-2xl border border-white/20 shadow-2xl">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3.5 bg-white rounded-xl text-gray-900 font-medium placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500"
                  />
                </div>
                <button
                  type="submit"
                  disabled={tracking}
                  className="flex items-center justify-center gap-2 bg-brand-500 hover:bg-brand-600 disabled:opacity-70 text-white font-bold px-8 py-3.5 rounded-xl transition-all shadow-lg shadow-brand-500/30 whitespace-nowrap"
                >
                  {tracking ? (
            ...
            <button
              ...
              className="mt-4 text-brand-400 hover:text-brand-300 text-sm underline underline-offset-2 transition-colors"
            >
              Try demo: {DEMO_ID}
            </button>
          </motion.div>
        ...
        {showResult && (
          ...
          >
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              {/* Status header */}
              <div className="bg-brand-500 text-white rounded-2xl px-8 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 shadow-xl shadow-brand-500/30">
                <div>
                  <p className="text-brand-100 text-sm mb-1">{t('tracking:status.title')}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {features.map((f, i) => (
              <motion.div
                ...
                whileHover={{ y: -8 }}
                className="group p-8 rounded-2xl border border-gray-100 hover:border-brand-200 hover:shadow-xl hover:shadow-brand-500/10 transition-all bg-white text-center"
              >
                <div className="w-16 h-16 bg-brand-50 group-hover:bg-brand-500 rounded-2xl flex items-center justify-center mx-auto mb-6 transition-colors duration-300">
